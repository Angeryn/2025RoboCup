...
void sw_i2c_init(void)
{
	...
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_OD;//*****OD（开漏）
	GPIO_InitStructure.GPIO_Pin = SW_I2C1_PIN_SCL | SW_I2C1_PIN_SDA;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(GPIOE, &GPIO_InitStructure);
	//配置B12B15为浮空输入（高阻态）防止对灰度信号造成干扰
...
//	GPIO_Init(GPIOB, &GPIO_InitStructure);
	...
	GPIO_SetBits(GPIOE, SW_I2C1_PIN_SCL | SW_I2C1_PIN_SDA);
}

/* 定义sda输出函数 bit=0为低电平 bit=1为高电平 */
void sda_out(uint8_t bit, void *user_data)
{
	GPIO_WriteBit(GPIOE, SW_I2C1_PIN_SDA, (BitAction)bit);
...
{
	uint8_t bit;
	bit = (uint8_t)GPIO_ReadInputDataBit(GPIOE, SW_I2C1_PIN_SDA);
...
{
	GPIO_WriteBit(GPIOE, SW_I2C1_PIN_SCL, (BitAction)bit);
...
void GW_digital_Init(void)
{
	...
	//4F
	/* 第一次IIC通讯会失败（因为软件IIC触发了start），手动发个stop也能消除 */
	sw_i2c_mem_read(&i2c_interface, 0x4F << 1, GW_GRAY_PING, &ping_response, 1);
	/* 后面IIC通讯是正常的 */
	sw_i2c_mem_read(&i2c_interface, 0x4F << 1, GW_GRAY_PING, &ping_response, 1);
	...
	sw_i2c_write_byte(&i2c_interface, 0x4F << 1, GW_GRAY_DIGITAL_MODE);
}
...
//2025//第二个八路灰度采用地址11（即AD0、AD1均连接跳线帽）
void GW_digital_Read2(uint8_t *digital_data)
{
	/* 读取开关量数据 */
	sw_i2c_read_byte(&i2c_interface, 0x4F << 1, digital_data); // digital_data 有1~8号探头开关数据
}
...
//2025
uint8_t GW_digital_Read_Bit2(uint8_t Num)
{
	uint8_t digital_data;
	GW_digital_Read2(&digital_data);
	// 从字节中提取第二个 比特
	return GET_NTH_BIT(digital_data, Num);
}

void GW_digital_Read_Array2(uint16_t *hd)//前八路
{
	uint8_t digital_data;
	GW_digital_Read2(&digital_data);
	for(int i=0;i<=7;i++)
	{
		hd[7-i]= GET_NTH_BIT(digital_data, i+1);//这里是反向赋值，原因是后面的灰度与前面灰度的朝向相反
	}
}

void GW_digital_Read_Array(uint16_t *hd)//后八路
{
	uint8_t digital_data;
	GW_digital_Read(&digital_data);
	for(int i=0;i<=7;i++)
	{
		hd[i]= GET_NTH_BIT(digital_data, i+1);
	}
}
...
/*以下written by Song /2024*/
/*每位数字单独读取成字符*/
void Grascale_Single_Read(char *digital_all_read)
{
	for(int i=1;i<=8;i++)
	{
		digital_all_read[i-1]=GW_digital_Read_Bit(i)+'0';
	}
}
