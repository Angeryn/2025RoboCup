...
#define SW_I2C1_PIN_SCL GPIO_Pin_1//根据实际情况定义I2C引脚，此处是GPIOE
...
void GW_digital_Read2(uint8_t *digital_data);//读第二个八路灰度的必要函数
uint8_t GW_digital_Read_Bit(uint8_t Num);//调用这个读单个数\2024
uint8_t GW_digital_Read_Bit2(uint8_t Num);//调用这个读第二个8路灰度单个数\2025

/*以数组形式一次性读八路值*/
void GW_digital_Read_Array(uint16_t *hd);//前八路
void GW_digital_Read_Array2(uint16_t *hd);//后八路

/*明天可以看一下是单个单个读与一次性读8路再定义区分哪个效率高，最好是结合双八路灰度逻辑思考一个最适合的底层灰度读取函数*/
...
void Grascale_Single_Read(char *digital_all_read);//调用这个读八位数字组成的字符数组\2024
